...
import { Cloud, Settings, Activity, Boxes, CloudCog, ChevronDown, ChevronRight, PanelLeftClose, PanelLeft, Network } from 'lucide-react';
  ...
  return (
    <aside className={`${isSidebarMinimized ? 'w-[72px]' : 'w-64'} transition-all duration-300 ease-in-out bg-[#0e1015] border-r border-[#1e232b] flex flex-col shrink-0 z-20 relative`}>
      ...
      <nav className="space-y-1 px-3">
        ...
        <div className="relative group/topology">
          <NavLink to="/topology" className={({isActive}) => `flex items-center gap-3 ${isSidebarMinimized ? 'justify-center' : ''} px-3 py-2.5 text-[13px] font-medium transition-colors border-l-2 ${isActive ? 'bg-[#1a1d24] text-white border-indigo-400 rounded-lg' : 'text-[#8b949e] hover:text-white hover:bg-[#1a1d24] border-transparent rounded-lg'}`}>
            {({isActive}) => (
              <>
                <Network className={`h-4 w-4 shrink-0 transition-colors ${isActive ? 'text-indigo-400' : 'text-[#8b949e] group-hover/topology:text-zinc-300'}`} />
                {!isSidebarMinimized && <span className="whitespace-nowrap overflow-hidden">Topology</span>}
              </>
            )}
          </NavLink>
          {isSidebarMinimized && (
            <div className="absolute left-[72px] top-1/2 -translate-y-1/2 ml-2 hidden group-hover/topology:block bg-[#15181e] text-white text-xs font-medium py-1.5 px-3 rounded-md shadow-xl border border-zinc-800 whitespace-nowrap z-[110]">
              Topology
            </div>
          )}
        </div>
