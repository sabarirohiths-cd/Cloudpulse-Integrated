...
export default function TopFilters({ topFilters, setTopFilters, selectedAccount, setSelectedAccount, accounts, availableRegions, availableLinked, availableTags }) {
  ...
  const filters = [
    { label: "Provider:", value: topFilters.provider, onChange: handleProviderChange, options: providerOptions, width: "w-[65px]" },
    { label: "Account:", value: selectedAccount, onChange: setSelectedAccount, options: filteredAccounts.map(a => ({ label: a.account_name, value: a.account_name })), width: "w-[100px]" },
    { label: "Region:", value: topFilters.region, onChange: v => setTopFilters({ ...topFilters, region: v }), options: [{ label: 'All Regions', value: 'All Regions' }, ...availableRegions.map(r => ({ label: r, value: r }))], width: "w-[80px]" },
    { label: linkedLabel, value: topFilters.linked, onChange: v => setTopFilters({ ...topFilters, linked: v }), options: [{ label: linkedOptionAll, value: 'All Accounts' }, ...availableLinked.map(l => ({ label: l, value: l }))], width: "w-[100px]" },
    { label: "Tag:", value: topFilters.tag, onChange: v => setTopFilters({ ...topFilters, tag: v }), options: [{ label: 'All', value: 'All' }, ...availableTags.map(t => ({ label: t, value: t }))], width: "w-[40px]" },
    {
      label: "Range:",
