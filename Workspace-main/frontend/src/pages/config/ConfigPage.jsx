...
import { Plus, Trash2, Clock, Cloud, MoreVertical, X, ShieldCheck, FileEdit, Server, Activity, CheckCircle2, Key, Network } from 'lucide-react';
...
const AVAILABLE_MODULES = [
  ...
  { id: 'topology', label: 'Topology', icon: Network, color: 'text-sky-400', border: 'border-sky-400/30', bg: 'bg-sky-400/10' }
];

const initialProviderForm = {
  activeProvider: 'aws',
  active_modules: ['inventory', 'control', 'topology'],
...
export default function ConfigPage() {
  ...
  const handleEditConfig = (c) => {
    ...
    const form = { ...initialProviderForm, activeProvider: c.provider.toLowerCase(), active_modules: (c.active_modules ?? 'inventory,control,topology').split(',').filter(Boolean) };
                        ...
                        <tr key={c.id} className="hover:bg-zinc-800/30 transition-colors group">
                          ...
                          <td className="px-4 py-3 align-middle">
                            <div className="flex flex-wrap gap-1.5">
                              {(c.active_modules ?? 'inventory,control,topology').split(',').filter(Boolean).map(m => {
