...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        try:
            await conn.execute(text("ALTER TABLE inventory_snapshots ADD COLUMN created_today INTEGER DEFAULT 0"))
        except Exception:
            pass

        try:
            await conn.execute(text("ALTER TABLE inventory_snapshots ADD COLUMN updated_today INTEGER DEFAULT 0"))
        except Exception:
            pass
