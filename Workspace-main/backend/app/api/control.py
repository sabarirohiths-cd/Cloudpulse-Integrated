...
async def list_audit_logs(
    ...
    ).where(ControlResource.is_visible == True)
...
async def _background_control_sync(account_name: Optional[str], region: str = "all") -> str:
    ...
    try:
            ...
            try:
                ...
                async with SessionLocal() as db:
                    ...
                    if db_config:
                        ...
                        db_config.last_sync_date = datetime.now().strftime("%b %d, %Y %I:%M %p")
                ...
                async with SessionLocal() as db:
                    if "ExpiredToken" in err_str or "RequestExpired" in err_str or "InvalidClientTokenId" in err_str or "InvalidAccessKeyId" in err_str:
                        ...
                        if db_config:
                            ...
                            if "ExpiredToken" in err_str or "RequestExpired" in err_str or ("InvalidClientTokenId" in err_str and creds.get("aws_session_token")):
                                db_config.last_error = "AWS Session Token has expired or is invalid."
                                error_messages.append(f"{config['account_name']}: AWS Session Token has expired or is invalid.")
                            else:
                                db_config.last_error = "AWS Access Key is invalid or deactivated."
                                error_messages.append(f"{config['account_name']}: AWS Access Key is invalid or deactivated.")
                            await db.commit()
                    else:
                        error_messages.append(f"{config['account_name']}: {err_str}")
                                
                    from app.models.system.system_notification import SystemNotification
                    notification = SystemNotification(
                        title="Control Sync Failed",
                        message=f"Failed to sync {config['account_name']}: AWS Session Token has expired or is invalid.",
                        type="ERROR",
                        module="CONTROL"
                    )
                    db.add(notification)
        ...
        async with SessionLocal() as db:
            if error_messages:
                return f"Failed sync for some accounts: {'; '.join(error_messages)}"
