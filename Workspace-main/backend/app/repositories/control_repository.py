...
class ControlRepository:
    ...
    async def update_visibility(db: AsyncSession, resource_ids: list[str], is_visible: bool):
        ...
        # Cascade UPWARDS: Check all parents of updated resources and sync their visibility
        parent_stmt = select(ControlResource.parent_resource_id).where(
            ControlResource.resource_id.in_(all_ids_to_update),
            ControlResource.parent_resource_id.isnot(None)
        ).distinct()
        parent_res = await db.execute(parent_stmt)
        parents_to_check = parent_res.scalars().all()
        
        for p_id in parents_to_check:
            # Check if ANY child of this parent is visible
            child_vis_stmt = select(ControlResource.is_visible).where(
                ControlResource.parent_resource_id == p_id,
                ControlResource.is_visible == True
            ).limit(1)
            any_visible = (await db.execute(child_vis_stmt)).scalars().first()
            
            await db.execute(
                update(ControlResource)
                .where(ControlResource.resource_id == p_id)
                .values(is_visible=bool(any_visible))
            )
