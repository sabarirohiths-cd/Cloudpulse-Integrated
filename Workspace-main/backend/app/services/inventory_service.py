...
async def generate_trend_snapshot(db: AsyncSession, provider: str, account_name: str, created_today: int = 0, updated_today: int = 0, change_breakdown: Dict[str, Dict[str, int]] = None):
    if change_breakdown is None:
        change_breakdown = {"created": {}, "deleted": {}, "updated": {}}
    ...
    # We no longer calculate total active breakdown here, as we are storing change_breakdown instead
    ...
    is_today = last_snapshot and last_snapshot.snapshot_date.strftime("%Y-%m-%d") == datetime.utcnow().strftime("%Y-%m-%d")

    accumulated_created = created_today
    accumulated_updated = updated_today
    accumulated_breakdown = change_breakdown

    if is_today:
        accumulated_created += last_snapshot.created_today
        accumulated_updated += last_snapshot.updated_today
        
        try:
            old_breakdown = json.loads(last_snapshot.category_breakdown) if last_snapshot.category_breakdown else {}
        except:
            old_breakdown = {}
            
        for action in ["created", "deleted", "updated"]:
            if action not in accumulated_breakdown: accumulated_breakdown[action] = {}
            old_action_data = old_breakdown.get(action, {})
            for r_type, count in old_action_data.items():
                accumulated_breakdown[action][r_type] = accumulated_breakdown[action].get(r_type, 0) + count
    ...
    if last_snapshot and last_snapshot.total_active_count == total:
        ...
        last_snapshot.deleted_today = deleted_today_count
        last_snapshot.created_today = accumulated_created
        last_snapshot.updated_today = accumulated_updated
        last_snapshot.category_breakdown = json.dumps(accumulated_breakdown)
        await db.commit()
        return
    ...
    snapshot = InventorySnapshot(
        ...
        created_today=accumulated_created,
        updated_today=accumulated_updated,
        category_breakdown=json.dumps(accumulated_breakdown),
...
async def sync_inventory(
    ...
    for native_id, item in fetched_map.items():
        ...
        if native_id not in db_map:
            ...
            db.add(change_log)
            metrics["created"] += 1
            change_breakdown["created"][item["type"]] = change_breakdown["created"].get(item["type"], 0) + 1
        else:
            ...
            if updated:
                metrics["updated"] += 1
                change_breakdown["updated"][db_item.resource_type] = change_breakdown["updated"].get(db_item.resource_type, 0) + 1
    ...
    for native_id, db_item in db_map.items():
        if native_id not in fetched_map and db_item.status == "active":
            ...
            db.add(change_log)
            metrics["deleted"] += 1
            change_breakdown["deleted"][db_item.resource_type] = change_breakdown["deleted"].get(db_item.resource_type, 0) + 1
    ...
    await generate_trend_snapshot(db, provider, account_name, metrics["created"], metrics["updated"], change_breakdown)
