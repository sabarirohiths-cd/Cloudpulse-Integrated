  ...
  const loadRegionsAndAccounts = useCallback(async (preserveView = false) => {
    try {
      ...
      if (regions && regions.length > 0) {
        ...
        setViewRegions(prev => {
          if (!preserveView && (!prev[0] || !regions.includes(prev[0]))) {
            return [regions[0]];
          }
          return prev;
        });
        ...
        setAvailableRegions([]);
        setViewRegions(prev => {
          if (!preserveView && prev.length > 0) return [];
          return prev;
        });
      }
  ...
  useEffect(() => {
    ...
    if (accountNames.length > 0) {
      ...
      if (defaultRegion && (!viewRegions[0] || !availableRegions.includes(viewRegions[0]))) {
        if (viewRegions.length === 0 || viewRegions[0] !== defaultRegion) {
  ...
  const fetchResources = useCallback(async (regionsToFetch = viewRegions) => {
    ...
    try {
      ...
      if (response && response.resources) {
        const totalErrors = response.summaries ? response.summaries.filter(s => s.error).length : 0;
        const totalRegions = response.summaries ? response.summaries.length : 0;
        
        // If every region failed (e.g. Expired Token), don't wipe the existing cached UI data
        if (totalRegions === 0 || totalErrors < totalRegions) {
