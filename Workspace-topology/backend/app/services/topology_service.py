    ...
    async def list_compute_resources(self, account_id: str, region: str, compute_type: str):
        ...
        if compute_type_upper == 'EC2':
            ...
            try:
                ...
                if "ExpiredToken" in err_str or "RequestExpired" in err_str:
                    raise Exception(f"Failed scanning {region}: AWS Session Token has expired or is invalid.")
                else:
                    raise Exception(f"Failed scanning {region}: {err_str}")
        else:
